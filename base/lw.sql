...
/*!50001 CREATE ALGORITHM = UNDEFINED */
  ...
  SELECT
    ...
    `usuario`.`contrasenia`        AS `contrasenia`,
    `usuario`.`email`              AS `email`,
    `usuario`.`last_login`         AS `last_login`,
    `usuario`.`rol`                AS `rol`,
    `info_usuario`.`nombres`       AS `nombres`,
    `info_usuario`.`apellidos`     AS `apellidos`,
    `info_usuario`.`numero_cursos` AS `numero_cursos`,
    `info_usuario`.`tag_line`      AS `tag_line`
  FROM (`usuario`
